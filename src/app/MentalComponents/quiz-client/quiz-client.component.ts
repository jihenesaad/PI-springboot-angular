...
import { MailgunService } from 'src/app/MentalService/mailgun.service';
...
export class QuizClientComponent {
  ...
  afficherChatbotFlag: boolean = false;
  ...
  constructor(private quizService: QuizService, private elementRef: ElementRef,private emailService: MailgunService,
     private login:AccountService, private noteService: NoteService) { }
...
showResult(): void {
  ...
  setTimeout(() => {
    ...
    const quizId = this.selectedQuiz?.idQuiz;
    const idUser = this.login.userid

    if (quizId) {
      this.quizService.calculateQuizScore(this.selectedAnswerIds, quizId, idUser).subscribe(
        (score: number) => {
          this.quizScore = score;
          if (this.quizScore >= 75 && this.quizScore <= 100) {
            this.afficherChatbot();
            
            this.emailService.sendEmail(idUser, 'Discover Our New Vitanova Programs!',
              `
              <div style="text-align: center;">
                <p style="font-size: 18px;"><strong>Hey! We are the Vitanova team.<br> We have created programs for you.</strong></p>
                <a href="http://localhost:4200/ProgramsForYou" style="display: inline-block;padding: 10px 20px; background-color: #FFA07A; color: #ffffff; text-decoration: none; border-radius: 5px;font-size: 15px;">Click here to check</a>
              </div>
              `
            ).subscribe(
              (response: any) => {
...
afficherChatbot(): void {
  this.afficherChatbotFlag = true;
}


isLastQuestion(): boolean {
  return this.questionsWithAnswers && this.currentQuestionIndex === this.questionsWithAnswers.length - 1;
}
